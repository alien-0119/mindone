...
from mindone.transformers.mindspore_adapter.utils import dtype_to_max, dtype_to_min
...
class EncoderRepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, penalty: float, encoder_input_ids: ms.Tensor):
        if not isinstance(penalty, float) or not (penalty > 0):
            raise ValueError(f"`penalty` has to be a strictly positive float, but is {penalty}")
    ...
    @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        score = mint.gather(scores, 1, self.encoder_input_ids)
...
class MinPLogitsWarper(LogitsProcessor):
    ...
    Created by @menhguin and @kalomaze (github handles). Code adapted from
    [this external PR](https://github.com/oobabooga/text-generation-webui/pull/4449/files)
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        sorted_indices = mint.argsort(scores, descending=True, dim=-1)
        sorted_indices_to_remove = mint.gather(tokens_to_remove, dim=-1, index=sorted_indices)
        # Keep at least min_tokens_to_keep
...
class TypicalLogitsWarper(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        # calculate entropy
        normalized = ms.mint.nn.functional.log_softmax(scores, dim=-1)
        p = mint.exp(normalized)
        ent = -(normalized * p).nansum(-1, keepdim=True)

        # shift and sort
        shifted_scores = mint.abs((-normalized) - ent)
        sorted_scores, sorted_indices = mint.sort(shifted_scores, descending=False)
...
class EpsilonLogitsWarper(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        indices_to_remove = indices_to_remove & (scores < mint.topk(scores, top_k)[0][..., -1, None])
...
class EtaLogitsWarper(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        probabilities = scores.softmax(dim=-1)
        entropy = nn.probability.distribution.Categorical(logits=scores).entropy()
        eta = mint.min(self.epsilon, mint.sqrt(self.epsilon) * mint.exp(-entropy))[..., None]
...
def _get_ngrams(ngram_size: int, prev_input_ids: ms.Tensor, num_hypos: int):
    """
...
class EncoderNoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, encoder_ngram_size: int, encoder_input_ids: ms.Tensor):
...
class SequenceBiasLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        for sequence_ids, sequence_bias in self.sequence_bias.items():
            ...
            matching_rows = mint.eq(
                input_ids[:, -prefix_length:],
                ms.tensor(sequence_ids[:-1], dtype=input_ids.dtype),
            ).prod(dim=1)
            bias[:, last_token] += mint.where(
                matching_rows.bool(),
                ms.tensor(sequence_bias),
                ms.tensor(0.0),
            )
    ...
    def _prepare_bias_variables(self, scores: ms.Tensor):
...
class HammingDiversityLogitsProcessor(LogitsProcessor):
    ...
    ['the solar system formed 4.6 billion years ago from the collapse of a giant interstellar molecular cloud. of the objects that orbit the Sun directly,
    the largest are the eight planets.',
    'the Solar System formed 4.6 billion years ago from the collapse of a giant interstellar molecular cloud. of the objects that orbit the Sun directly,
    the largest are the eight planets.']
    ...
    the largest are the eight planets.',
    'the solar system formed 4.6 billion years ago from the collapse of a giant interstellar molecular cloud. of the objects that orbit the Sun directly,
    the largest are the eight planets. the rest of the objects are smaller objects, such as the five dwarf planets and small solar system bodies.']
    ```
    """
    ...
    def __call__(
        ...
        Args:
            ...
            scores (`ms.Tensor` of shape `(batch_size, config.vocab_size)`):
                Prediction scores of a language modeling head. These can be logits for each vocabulary when not using
                beam search or log softmax for each vocabulary token when using beam search
            current_tokens (`ms.Tensor` of shape `(batch_size)`):
        ...
        for batch_idx in range(batch_size):
            ...
            token_frequency = mint.bincount(previous_group_tokens, minlength=vocab_size)
...
class ForcedBOSTokenLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        scores_processed = scores
        if cur_len == 1:
            scores_processed = mint.full_like(scores, -math.inf)
...
class InfNanRemoveLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        scores_processed = mint.where(scores != scores, 0.0, scores)

        # set all +/-inf values to max/min possible value
        scores_processed = mint.where(scores == float("inf"), dtype_to_max(scores.dtype), scores_processed)
        scores_processed = mint.where(scores == -float("inf"), dtype_to_min(scores.dtype), scores_processed)
...
class ExponentialDecayLengthPenalty(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        penalties = mint.zeros_like(scores)
        scores_processed = scores
        if cur_len > self.regulation_start:
            ...
            penalty = mint.abs(scores[:, self.eos_token_id]) * (pow(self.regulation_factor, penalty_idx) - 1)
...
class SuppressTokensAtBeginLogitsProcessor(LogitsProcessor):
    ...
    @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        vocab_tensor = mint.arange(
            scores.shape[-1],
        )
        suppress_token_mask = mnp.isin(vocab_tensor, self.begin_suppress_tokens)
        scores_processed = scores
...
class SuppressTokensLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, suppress_tokens):
        self.suppress_tokens = ms.tensor(
            list(suppress_tokens),
        )
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        suppress_token_mask = mnp.isin(vocab_tensor, self.suppress_tokens)
    ...
    def get_unconditional_logits(self, input_ids):
        if self.unconditional_context["first_pass"]:
            ...
            if self.unconditional_context["attention_mask"] is None:
                self.unconditional_context["attention_mask"] = mint.ones_like(
            ...
            attention_mask = mint.cat(
                [
                    ...
                    mint.ones_like(input_ids[:, -1:], dtype=ms.int32),
                ],
                dim=1,
            )
...
class BarkEosPrioritizerLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        scores_processed = scores
        if self.min_eos_p:
            probs = mint.nn.functional.softmax(scores.float(), dim=-1)
            # create scores full of -inf except for the eos_token_id
            early_stop_scores = mint.ones_like(scores) * -float("inf")
            early_stop_scores[:, self.eos_token_id] = scores[:, self.eos_token_id]

            do_early_stop = probs[:, self.eos_token_id] > self.min_eos_p
            do_early_stop = mint.any(do_early_stop, dim=1, keepdim=True)
            scores_processed = mint.where(do_early_stop, early_stop_scores, scores)
...
class WatermarkLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        self.fixed_table = mint.randperm(
            self.table_size,
            generator=self.rng,
        )

    def set_seed(self, input_seq: ms.Tensor):
    ...
    def _get_greenlist_ids(self, input_seq: ms.Tensor) -> ms.Tensor:
        self.set_seed(input_seq)
        vocab_permutation = mint.randperm(self.vocab_size, generator=self.rng)
        greenlist_ids = vocab_permutation[: self.greenlist_size]
        return greenlist_ids

    def _score_rejection_sampling(self, input_seq: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        """
        ...
        for i in range(40):
            greenlist_ids = self._get_greenlist_ids(mint.cat([input_seq, greedy_predictions[i, None]], dim=-1))
            if greedy_predictions[i] in greenlist_ids:
                final_greenlist.append(greedy_predictions[i])
        return ms.tensor(final_greenlist)
...
class SynthIDTextWatermarkState:
    ...
    def __init__(
        ...
        """
        self.context = mint.zeros(
            (batch_size, ngram_len - 1),
            dtype=ms.int64,
        )
        self.context_history = mint.zeros(
...
class SynthIDTextWatermarkLogitsProcessor(LogitsProcessor):
    ...
    * **Utility Functions:** Provides functions to compute g-values directly, check for context repetition, create
    ...
    >>> tokenized_prompts = tokenizer(["Once upon a time, "], return_tensors="np", padding=True)
    ...
    def __init__(
        ...
        generator = ms.Generator().manual_seed(sampling_table_seed)
        ...
        self.sampling_table = mint.randint(
            low=0,
            high=2,
    ...
    def update_scores(self, scores: ms.Tensor, g_values: ms.Tensor) -> ms.Tensor:
        """Updates scores using the g values.

        We assume that the scores are in the log space.
        Args:
            scores (`ms.Tensor`): Scores (batch_size, vocab_size).
            g_values (`ms.Tensor`): G valus (batch_size, vocab_size, depth).
        ...
        log_probs = mint.where(mint.isfinite(log_probs), log_probs, dtype_to_min(log_probs.dtype))
        return log_probs
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        if self.debug_mode:
            scores = mint.ones_like(scores)

        # Currently indices is just a arange to compute watermarking on the desnse logits.
        all_indices = mint.stack([mint.arange(vocab_size) for _ in range(batch_size)])
        ...
        if self.state is None:
            ...
            # watermarking.
            self.state.context = mint.concat(
        ...
        self.state.context_history = mint.concat(
            (hash_result_with_just_context, self.state.context_history),
            dim=1,
        )[:, :-1]

        updated_watermarked_scores = mint.where(
            is_repeated_context,
            input=scores,
    ...
    def accumulate_hash(
        ...
        for i in range(data.shape[-1]):
            current_hash = mint.add(current_hash, data[..., i])
            current_hash = mint.mul(current_hash, multiplier)
            current_hash = mint.add(current_hash, increment)
        return current_hash

    def compute_ngram_keys(self, ngrams: ms.Tensor) -> ms.Tensor:
        ...
        hash_result = mint.ones(batch_size, dtype=ms.int64)
        # hash_result shape [batch_size,]
        # ngrams shape [batch_size, num_ngrams, ngram_len]
        hash_result = ms.vmap(self.accumulate_hash, in_axes=(None, 1), out_axes=1)(hash_result, ngrams)
        # hash_result shape [batch_size, num_ngrams]

        keys = self.keys[None, None, :, None]
        # hash_result shape [batch_size, num_ngrams]
        # keys shape [1, 1, depth, 1]
        hash_result = ms.vmap(self.accumulate_hash, in_axes=(None, 2), out_axes=2)(hash_result, keys)
        # hash_result shape [batch_size, num_ngrams, depth]

        return hash_result

    def _compute_keys(self, n_minus_1_grams: ms.Tensor, indices: ms.Tensor) -> tuple[ms.Tensor, ms.Tensor]:
        """Computes random keys for each ngram and depth.

        Args:
            n_minus_1_grams (`ms.Tensor`):
        ...
        hash_result = ms.vmap(self.accumulate_hash, in_axes=(None, 1), out_axes=1)(
    ...
    def sample_g_values(self, ngram_keys: ms.Tensor) -> ms.Tensor:
        """
        Samples g values from Bernoulli distribution.

        It is not possible to pass random keys in a vectorized way in MindSpore. Instead
        we pre-compute a random sampling table, and use apply modulo table size to
        map from ngram keys (int64) to g values.

        Args:
            ngram_keys (`ms.Tensor`):
        ...
        return mint.take_along_dim(sampling_table, indices=ngram_keys, dim=2)

    def _check_input_ids_shape(self, input_ids: ms.Tensor):
    ...
    def compute_g_values(self, input_ids: ms.Tensor) -> ms.Tensor:
        """
    ...
    def compute_context_repetition_mask(self, input_ids: ms.Tensor) -> ms.Tensor:
        """
        ...
        for i in range(num_contexts):
            ...
            state.context_history = mint.concat(
                (context_hash, state.context_history),
                dim=1,
            )[:, :-1]
        are_repeated_contexts = mint.concat(are_repeated_contexts, dim=1)

        return mint.logical_not(are_repeated_contexts)

    def compute_eos_token_mask(self, input_ids: ms.Tensor, eos_token_id: int) -> ms.Tensor:
        """
        ...
        for eos_equated in all_eos_equated:
            nonzero_idx = mint.nonzero(eos_equated)
            noneos_mask = mint.ones_like(eos_equated)
            if nonzero_idx.shape[0] != 0:
                noneos_mask[nonzero_idx[0][0] :] = 0
            noneos_masks.append(noneos_mask)
        return mint.stack(noneos_masks, dim=0)
