...
from transformers import GenerationConfig, KyutaiSpeechToTextConfig, logging
...
class KyutaiSpeechToTextRMSNorm(nn.Cell):
    def __init__(self, dim: int, eps: float = 1e-6):
        super().__init__()
        self.eps = eps
        self.weight = Parameter(mint.ones(dim))  # Ignore copy
...
class KyutaiSpeechToTextFlexibleLinear(nn.Cell):
    def __init__(self, input_size, output_size, num_layers):
        super().__init__()
        # Stack the weights for N layers into a single tensor (num_layers, output_size, input_size)
        self.weight = Parameter(mint.randn(num_layers, output_size, input_size))

    def construct(self, x, layer_idx=None):
        """
        `KyutaiSpeechToTextFlexibleLinear` creates one linear layer per codebook. There's multiple ways to use it.
        In the default case, `sequence_length=num_layers`,
        so each element of the sequence will be matmul to the weights corresponding to its index on the sequence.
        ...
        Args:
            x (`ms.Tensor): input to the layer of shape `(batch, num_layers, embed_dim)` or of shape `(batch, seq_length, embed_dim)`
            layer_idx (`ms.Tensor`, *optional*):
                ...
                But if `layer_idx` is a tensor of shape `(seq_length,)`,
                it will matmul each i-th element of the input sequence to the corresponding layer `weight[i]`.
        """

        # Use mint.index_select to select the corresponding weights for each sample
        # (codebooks, output_size, hidden_size)
        selected_weights = mint.index_select(self.weight, 0, layer_idx) if layer_idx is not None else self.weight
        ...
        x = mint.matmul(x[:, :, None, :], selected_weights)
...
class KyutaiSpeechToTextConv1dPaddingCache:
    ...
    def update(self, hidden_states: Tensor, layer_idx: int):
        ...
        Returns:
            `ms.Tensor` or `None`, the current padding cache.
        """
        batch_size, dtype = (
...
class KyutaiSpeechToTextEmbeddings(nn.Cell):
    def __init__(self, config):
        ...
        audio_tokens_offsets = mint.arange(config.num_codebooks) * config.codebook_vocab_size
        audio_tokens_offsets += config.vocab_size
        audio_tokens_offsets = mint.nn.functional.pad(
...
class KyutaiSpeechToTextLinear(nn.Cell):
    def __init__(self, input_dim, output_dim, num_codebooks, use_flexible_linear=False):
        ...
        if not use_flexible_linear:
            self.linear = mint.nn.Linear(input_dim, output_dim, bias=False)
        else:
...
class KyutaiSpeechToTextGatingMLP(nn.Cell):
    def __init__(self, config, use_flexible_linear=False):
        ...
        if num_layers == 1:
            self.fc1 = mint.nn.Linear(hidden_size, ffn_dim, bias=False)
            self.fc2 = mint.nn.Linear(ffn_dim // 2, hidden_size, bias=False)
        else:
    ...
    def construct(self, hidden_states: Tensor, layer_idx: Optional[int] = None) -> Tensor:
...
class KyutaiSpeechToTextAttention(nn.Cell):
...
class KyutaiSpeechToTextSdpaAttention(KyutaiSpeechToTextAttention):
    """
    KyutaiSpeechToText attention module using scaled_dot_product_attention. This module inherits from
...
class KyutaiSpeechToTextForConditionalGeneration(KyutaiSpeechToTextPreTrainedModel, GenerationMixin):
    ...
    def _prepare_model_inputs(
        ...
        model_kwargs["audio_tokens"] = mint.zeros(
            (batch_size, audio_window_size, self.config.num_codebooks),
            dtype=mindspore.int64,
        )
        model_kwargs["current_window"] = Tensor([0, 0], dtype=mindspore.int64).expand((batch_size, -1)).contiguous()
        ...
        for layer_name in self.codec_model.encoder._mimiconv1d_layer_names:
            per_layer_padding.append(self.codec_model.encoder.get_sub_cell(layer_name).padding_total)
            per_layer_padding_mode.append(self.codec_model.encoder.get_sub_cell(layer_name).pad_mode)
            per_layer_in_channels.append(self.codec_model.encoder.get_sub_cell(layer_name).in_channels)
        ...
        **kwargs,
        ...
        if input_values is not None:
            ...
            if cache_position[-1] - 1 >= end:
                ...
                current_window.copy_(Tensor([start, end]).expand((current_window.shape[0], -1)))
            ...
            input_ids = mint.cat(
                [input_ids.unsqueeze(2), current_audio_tokens.to(input_ids.dtype)],
                dim=2,
            )
    ...
    def generate(self, *args, **kwargs):
        r"""
        This method forwards all its arguments to GenerationMixin's [`~GenerationMixin.generate`].
