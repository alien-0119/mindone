...
class BartLearnedPositionalEmbedding(mint.nn.Embedding):
    ...
    def construct(self, input_ids: ms.Tensor, past_key_values_length: int = 0, position_ids: Optional[ms.Tensor] = None):
        """`input_ids' shape is expected to be [bsz x seqlen]."""

        if position_ids is None:
            bsz, seq_len = input_ids.shape[:2]
            position_ids = mint.arange(past_key_values_length, past_key_values_length + seq_len, dtype=ms.int32).expand((bsz, -1))
        else:
...
class BartPreTrainedModel(PreTrainedModel):
    ...
    def _update_full_mask(
        self,
        attention_mask: Union[ms.Tensor, None],
    ...
    def _update_causal_mask(
        self,
        attention_mask: Optional[Union[ms.Tensor]],
    ...
    def _update_cross_attn_mask(
        self,
        encoder_hidden_states: Union[ms.Tensor, None],
        encoder_attention_mask: Union[ms.Tensor, None],
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            ...
            # elif self.config._attn_implementation == "flex_attention":
            #     if isinstance(encoder_attention_mask, ms.Tensor):
...
class BartEncoder(BartPreTrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[mint.nn.Embedding] = None):
        ...
        self.layers = nn.CellList([BartEncoderLayer(config, layer_idx=i) for i in range(config.encoder_layers)])
...
class BartDecoder(BartPreTrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[mint.nn.Embedding] = None):
        ...
        self.layers = nn.CellList([BartDecoderLayer(config, layer_idx=i) for i in range(config.decoder_layers)])
    ...
    def construct(
        ...
        if labels is not None:
            ...
            if self.config.problem_type is None:
                ...
                elif self.config.num_labels > 1 and (labels.dtype == ms.int32 or labels.dtype == ms.int64):
