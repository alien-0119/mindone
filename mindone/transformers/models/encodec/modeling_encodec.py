...
from mindspore.common.initializer import HeNormal, initializer, Uniform
...
class EncodecOutput(ModelOutput):
    """
    Args:
        audio_codes (`ms.Tensor`  of shape `(batch_size, nb_chunks, chunk_length)`, *optional*):
...
class EncodecEncoderOutput(ModelOutput):
    ...
    Args:
        audio_codes (`ms.Tensor`  of shape `(batch_size, nb_chunks, chunk_length)`, *optional*):
            Discret code embeddings computed using `model.encode`.
        audio_scales (`ms.Tensor` of shape `(batch_size, nb_chunks)`, *optional*):
            Scaling factor for each `audio_codes` input. This is used to unscale each chunk of audio when decoding.
    """

    audio_codes: Tensor = None
    audio_scales: Tensor = None
...
class EncodecConv1d(nn.Cell):
    ...
    def __init__(
        ...
        if self.norm_type == "weight_norm":
            ...
            self.norm = mint.nn.GroupNorm(1, out_channels)
...
class EncodecEuclideanCodebook(nn.Cell):
    ...
    def __init__(self, config: EncodecConfig):
        ...
        self.register_buffer("inited", Tensor([True]))
        self.register_buffer("cluster_size", mint.zeros(config.codebook_size))
    ...
    def quantize(self, hidden_states):
        ...
        # FIXME: tensor.max of mindspore and torch has different output type
        # embed_ind = dist.max(dim=-1).indices
        embed_ind = dist.max(dim=-1)[1]
        return embed_ind
...
class EncodecResidualVectorQuantizer(nn.Cell):
    ...
    def encode(self, embeddings: Tensor, bandwidth: Optional[float] = None) -> Tensor:
        """
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, mint.nn.Linear):
            ...
            if module.bias is not None:
                ...
                module.bias.set_data(initializer(Uniform(k), module.shape, module.dtype))
...
class EncodecModel(EncodecPreTrainedModel):
    ...
    def _encode_frame(
        self, input_values: Tensor, bandwidth: float, padding_mask: int
        ...
        if self.config.normalize:
            ...
            mono = mint.sum(input_values, 1, keepdim=True) / input_values.shape[1]
    ...
    def encode(
        ...
        encoded_frames = mint.stack(encoded_frames)
    ...
    def _decode_frame(self, codes: Tensor, scale: Optional[Tensor] = None) -> Tensor:
        codes = codes.transpose(0, 1)
    ...
    def decode(
        ...
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[Tensor, Tensor], EncodecDecoderOutput]:
        """
    ...
    def construct(
        ...
        audio_scales: Optional[Tensor] = None,
        return_dict: Optional[bool] = None,
