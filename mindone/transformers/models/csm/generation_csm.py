...
class CsmGenerateOutput(GenerateDecoderOnlyOutput):
    ...
    Args:
        ...
        audio (`list(ms.Tensor)` of length `batch_size`):
            The generated audio.
    """

    audio: Optional[list[Tensor]] = None
...
class CsmGenerationMixin(GenerationMixin):
    ...
    def _sample(
        ...
        **model_kwargs,
    ) -> Union[GenerateNonBeamOutput, Tensor]:
        """
        ...
        unfinished_sequences = mint.ones(batch_size, dtype=mindspore.int64)
        ...
        while self._has_unfinished_sequences(
            ...
            depth_decoder_input_ids = mint.nn.functional.pad(first_codebook_ids, (1, 0), value=0)
            ...
            codebook_ids = (
                depth_decoder_outputs if isinstance(depth_decoder_outputs, Tensor) else depth_decoder_outputs.sequences
            )
    ...
    def generate(
        ...
        input_values_cutoffs: Optional[Tensor] = None,
        ...
        **kwargs,
        ...
        Parameters:
            inputs_ids (`ms.Tensor` of shape (batch_size, seq_length), *optional*):
                The sequence used as a prompt for the backbone model.
            input_values (`ms.Tensor` of shape (batch_size, channels, max_concatenated_audio_length), *optional*):
            ...
            input_values_cutoffs (`ms.Tensor` of shape (batch_size, max_num_audio), *optional*):
        ...
        Return:
            [`CsmGenerateOutput`] or `ms.Tensor` or `list[ms.Tensor]`: A [`CsmGenerateOutput`]
            (if `return_dict_in_generate=True` or when `config.return_dict_in_generate=True`) or a `ms.Tensor` when `output_audio=False`
            or a `list[ms.Tensor]` otherwise.

        Example:

        ```python
        >>> from transformers import CsmProcessor
        >>> from mindone.transformers import CsmForConditionalGeneration
        ...
        generate_returned_dict = not isinstance(generate_output, Tensor)
        audio = None
        if output_audio:
