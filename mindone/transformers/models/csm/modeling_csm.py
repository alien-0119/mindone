...
class CsmOutputWithPast(ModelOutput):
    ...
    depth_decoder_loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when `labels` is provided):
        Language modeling loss (for next-token prediction) of the depth decoder model.
    depth_decoder_logits (`ms.Tensor` of shape `(batch_size, sequence_length, config.vocab_size)`):
    ...
    depth_decoder_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
        Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
        sequence_length)`.
    backbone_loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when `labels` is provided):
    ...
    depth_decoder_loss: Optional[Tensor] = None
    depth_decoder_logits: Tensor = None
    depth_decoder_past_key_values: Optional[tuple[tuple[Tensor]]] = None
    depth_decoder_hidden_states: Optional[tuple[Tensor, ...]] = None
    depth_decoder_attentions: Optional[tuple[Tensor, ...]] = None
...
class CsmRotaryEmbedding(nn.Cell):
...
class CsmDecoderLayer(nn.Cell):  # TODO: GradientCheckpointingLayer
    def __init__(self, config: CsmConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class CsmDepthDecoderModel(CsmPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embed_tokens = mint.nn.Embedding((config.num_codebooks * config.vocab_size), config.backbone_hidden_size)
        self.layers = nn.CellList([CsmDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self.norm = CsmRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = CsmRotaryEmbedding(config=config)
        self.gradient_checkpointing = False
        self.inputs_embeds_projector = mint.nn.Linear(config.backbone_hidden_size, config.hidden_size, bias=False)
    ...
    def construct(
        ...
        backbone_last_hidden_state: Optional[Tensor] = None,
        attention_mask: Optional[Tensor] = None,
        position_ids: Optional[Tensor] = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
        backbone_last_hidden_state (`ms.Tensor` of shape `(batch_size, backbone_hidden_size)`, *optional*):
        ...
        if cache_position is None:
            ...
            cache_position = mint.arange(past_seen_tokens, past_seen_tokens + inputs_seq_length)

        if inputs_embeds is None:
            codebook_idxs = mint.clamp(cache_position - 1, min=0)
...
class CsmCodebooksHead(nn.Cell):
    def __init__(self, hidden_size, num_codebooks, vocab_size):
        super().__init__()
        self.num_codebooks = num_codebooks
        self.weight = Parameter(mint.empty(self.num_codebooks - 1, hidden_size, vocab_size))

    def construct(self, hidden_states, cache_position=None):
        if cache_position is None:
            seq_length = hidden_states.shape[1]
            codebook_weight = self.weight[mint.arange(seq_length)]
        else:
...
class CsmBackboneModelEmbeddings(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.embed_audio_tokens = mint.nn.Embedding((config.num_codebooks * config.vocab_size), config.hidden_size)
        self.register_buffer(
            "audio_tokens_offsets", mint.arange(config.num_codebooks) * config.vocab_size, persistent=False
        )
...
class CsmBackboneModel(CsmPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
        input_ids (`ms.Tensor` of shape `(batch_size, sequence_length, num_codebooks) or (batch_size, sequence_length)`):
        ...
        if cache_position is None:
            past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
            cache_position: Tensor = mint.arange(past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1])
...
class CsmForConditionalGeneration(CsmPreTrainedModel, CsmGenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.embed_text_tokens = mint.nn.Embedding(config.text_vocab_size, config.hidden_size)
    ...
    def from_pretrained(cls, *args, **kwargs):
        ...
        depth_decoder_attrs = {
            attr[prefix_len:]: value for attr, value in vars(model.generation_config).items() if attr.startswith(prefix)
        }
    ...
    def _merge_input_ids_with_input_values(
        ...
        Args:
            ...
            input_values (`ms.Tensor` of shape `(batch_size, channels, audio_sequence_length)`):
                The audio input values to embed.
            input_values_cutoffs (`ms.Tensor` of shape `(batch_size, max_num_audio)`):
        ...
        if input_values is not None:
            ...
            input_values_cutoffs = mint.nn.functional.pad(input_values_cutoffs, (1, 0))
            audio_lengths = input_values_cutoffs[input_values_cutoffs >= 0].diff()
            audio_lengths = audio_lengths[audio_lengths > 0]
            input_values_mask = mint.arange(input_values_cutoffs.max().tolist()).expand((len(audio_lengths), -1))
            ...
            with mindspore._no_grad():
                ...
                batched_audio_token_ids = mint.stack(
                    [mint.nn.functional.pad(el, (0, 0, 0, max_audio_frames - el.shape[0])) for el in audio_tokens_list]
                )
            ...
            audio_eos_frame_ids = (
                mint.ones((1, 1, self.config.num_codebooks), dtype=mindspore.int64) * self.config.codebook_eos_token_id
            )
            ...
            inputs_embeds[audio_eos_token_mask] = audio_eos_embeds.repeat(audio_eos_token_mask.sum().tolist(), 1)
            ...
            if labels is not None:
                ...
                # FIXME: ms 2.6/2.7 does not support tensors whose indice is a tensor with booleans elements.
                #  It has been fixed in ms 2.7.1.
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if labels is not None:
            ...
            depth_decoder_input_ids = mint.nn.functional.pad(depth_decoder_input_ids, (1, 0), value=0)
        ...
        return CsmOutputWithPast(
            ...
            depth_decoder_past_key_values=(
                depth_decoder_outputs.past_key_values if depth_decoder_outputs is not None else None
            ),
            depth_decoder_hidden_states=(
                depth_decoder_outputs.hidden_states if depth_decoder_outputs is not None else None
            ),
