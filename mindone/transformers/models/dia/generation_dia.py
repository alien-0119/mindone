...
from ...generation.utils import GenerateOutput, GenerationMixin
...
class DiaGenerationMixin(GenerationMixin):
    ...
    def _prepare_model_inputs(
        ...
        if self._uses_cfg:
            unconditioned_inputs = mint.zeros_like(inputs)
            inputs = mint.cat([inputs, unconditioned_inputs], dim=0)
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        if decoder_input_ids is None or decoder_attention_mask is None:
            ...
            if decoder_input_ids is None:
                decoder_input_ids = mint.full(
                    (real_batch_size, 1, num_channels), decoder_start_token_id, dtype=mindspore.int64
                )

            decoder_attention_mask = mint.ones(
                size=(real_batch_size, decoder_input_ids.shape[1]), dtype=mindspore.int64
            )
    ...
    @staticmethod
    def apply_delay_mask(input_ids: Tensor, pad_id: int, delay_mask: Optional[Tensor]) -> Tensor:
        ...
        input_ids[:, :mask_len, :] = mint.where(valid_mask == pad_id, valid_input, valid_mask)
    ...
    def _main_generate_loop(
        ...
        **kwargs,
        ...
        if synced_gpus is None:
            synced_gpus = (is_deepspeed_zero3_enabled() or is_fsdp_managed_module(self)) and mint.distributed.get_world_size() > 1
    ...
    def generate(
        ...
        **kwargs,
        ...
        return_dict_in_generate = not isinstance(output, Tensor)
