...
class DiaProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs: Unpack[DiaProcessorKwargs],
        ...
        if audio is not None:
            ...
            # FIXME: return_tensor can only be np and will be used by mindspore api, so we need to turn np to ms
            input_audios = {k: mindspore.tensor(v) for k, v in input_audios.items()}
            ...
            for padding_mask, audio in zip(input_audios["padding_mask"], input_audios["input_values"]):
                ...
                attention_mask = Tensor([0] * padding_len + [1] * num_valid_inputs, dtype=mindspore.long)[None, :]

                decoder_input_ids.append(input_ids)
                decoder_attention_mask.append(attention_mask)

            decoder_input_ids = mint.cat(decoder_input_ids, dim=0)
            decoder_attention_mask = mint.cat(decoder_attention_mask, dim=0)
        # TTS generation
        elif generation:
            # all bos to start with TTS
            decoder_input_ids = mint.full((batch_size, 1, num_channels), audio_bos_token_id, dtype=mindspore.long)

            # we preemptively add the delay
            decoder_attention_mask = mint.ones(size=(batch_size, 1 + max_delay), dtype=mindspore.long)
        else:
    ...
    def batch_decode(
        ...
        **kwargs: Unpack[DiaProcessorKwargs],
        ...
        Args:
            decoder_input_ids (`ms.Tensor`): The complete output sequence of the decoder.
        ...
        if audio_prompt_len is not None:
            audio_prompt_len = Tensor(audio_prompt_len, dtype=mindspore.long)
        ...
        with mindspore._no_grad():
            for i in range(start_of_generation_idx.shape[0]):
                ...
                audio_i = self.audio_tokenizer.decode(audio_codes=output_i).audio_values.squeeze()
    ...
    def save_audio(
        ...
        **kwargs: Unpack[DiaProcessorKwargs],
        ...
        for audio_value, p in zip(audio, saving_path):
            if isinstance(audio_value, Tensor):
                audio_value = audio_value.float().numpy()
    ...
    def build_indices(
        ...
        """
        delay_array = Tensor(delay_pattern, dtype=mindspore.int32)

        # (0..seq_len-1)
        sequence_idx = mint.arange(seq_len, dtype=mindspore.int32)[None, :].expand((bsz, seq_len))[..., None]
        ...
        valid_sequence_idx = mint.clamp(sequence_idx, 0, seq_len - 1)

        batch_idx = mint.arange(bsz, dtype=mindspore.int32)[:, None, None].expand((bsz, seq_len, num_channels))
        channel_idx = mint.arange(num_channels, dtype=mindspore.int32)[None, None, :].expand((bsz, seq_len, num_channels))
    ...
    def apply_audio_delay(
        ...
        pad_token_id: int,
        bos_token_id: int,
        precomputed_idx: tuple["Tensor", "Tensor"],
    ) -> "Tensor":
        """
        ...
        batch_idx, valid_sequence_idx, channel_idx = mint.unbind(all_idx, dim=-1)
        gathered_audio = audio[batch_idx, valid_sequence_idx, channel_idx].view(audio.shape)
        ...
        final_audio = mint.where(mask_bos, bos_token_id, mint.where(mask_pad, pad_token_id, gathered_audio))
