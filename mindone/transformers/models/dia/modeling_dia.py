...
class DiaMultiChannelEmbedding(nn.Cell):
    ...
    def __init__(self, config: DiaDecoderConfig):
        super().__init__()
        self.embed = mint.nn.Embedding(config.vocab_size * config.num_channels, config.hidden_size)
        self.hidden_size = config.hidden_size
        self.num_channels = config.num_channels
        offsets = mint.arange(config.num_channels, dtype=mindspore.int64) * config.vocab_size  # (C,)
        self.register_buffer("offsets", offsets, persistent=False)

    def construct(self, audio_codes: Tensor) -> Tensor:
...
class DiaMLP(nn.Cell):
    def __init__(self, config):
        super().__init__()

        self.config = config
        self.gate_up_proj = mint.nn.Linear(config.hidden_size, 2 * config.intermediate_size, bias=False)
        self.down_proj = mint.nn.Linear(config.intermediate_size, config.hidden_size, bias=False)
...
class DiaRotaryEmbedding(nn.Cell):
...
class DiaSelfAttention(nn.Cell):
...
class DiaCrossAttention(nn.Cell):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def __init__(self, config: DiaDecoderConfig, layer_idx: int):
        ...
        self.q_proj = mint.nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=False)
        self.k_proj = mint.nn.Linear(self.cross_hidden_size, self.num_key_value_heads * self.head_dim, bias=False)
        self.v_proj = mint.nn.Linear(self.cross_hidden_size, self.num_key_value_heads * self.head_dim, bias=False)
        self.o_proj = mint.nn.Linear(self.num_heads * self.head_dim, self.hidden_size, bias=False)
...
class DiaEncoder(DiaPreTrainedModel):
    def __init__(self, config: DiaEncoderConfig):
        ...
        self.embedding = mint.nn.Embedding(config.vocab_size, config.hidden_size)
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        position_ids = mint.arange(input_ids.shape[-1])[None, :]
    ...
    def _update_full_mask(
        self,
        attention_mask: Union[Tensor, None],
...
class DiaDecoder(DiaPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs,
        ...
        r"""
        input_ids (`ms.Tensor` of shape `(batch_size, sequence_length, num_codebooks)`):
    ...
    def _update_cross_attn_mask(
        self,
        encoder_hidden_states: Union[Tensor, None],
        encoder_attention_mask: Union[Tensor, None],
        input_shape: list,
        inputs_embeds: Tensor,
    ):
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            ...
            # elif self.config._attn_implementation == "flex_attention":
            #     if isinstance(encoder_attention_mask, Tensor):
...
class DiaModel(DiaPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs,
        ...
        r"""
        decoder_input_ids (`ms.Tensor` of shape `(batch_size * num_codebooks, target_sequence_length)
        ...
        decoder_position_ids (`ms.Tensor` of shape `(batch_size, target_sequence_length)`):
        ...
        if self._supports_gradient_checkpointing and self.training:
            if use_cache:
                logger.warning_once(
        ...
        if decoder_input_ids is None:
            decoder_input_ids = mint.full(
                size=(bsz, 1, channels), fill_value=self.config.bos_token_id
            )
        # Ensure 3D
...
class DiaForConditionalGeneration(DiaPreTrainedModel, DiaGenerationMixin):
    ...
    def __init__(self, config: DiaConfig):
        ...
        self.logits_dense = mint.nn.Linear(
    ...
    def construct(
        ...
        **kwargs,
        ...
        decoder_position_ids (`ms.Tensor` of shape `(batch_size, target_sequence_length)`):
            ...
            [What are position IDs?](../glossary#position-ids)
        labels (`ms.Tensor` of shape `(batch_size * num_codebooks,)`, *optional*):
