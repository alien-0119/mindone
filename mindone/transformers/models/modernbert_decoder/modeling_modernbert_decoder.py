...
class ModernBertDecoderAttention(nn.Cell):
    ...
    def __init__(self, config: ModernBertDecoderConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = mint.nn.Linear(self.config.hidden_size, self.all_head_size, bias=self.config.attention_bias)
        self.k_proj = mint.nn.Linear(self.config.hidden_size, self.all_head_size, bias=self.config.attention_bias)
        self.v_proj = mint.nn.Linear(self.config.hidden_size, self.all_head_size, bias=self.config.attention_bias)

        self.Wo = mint.nn.Linear(config.hidden_size, config.hidden_size, bias=config.attention_bias)
        self.out_drop = mint.nn.Dropout(config.attention_dropout)
...
class ModernBertDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: ModernBertDecoderConfig, layer_idx: Optional[int] = None):
        ...
        self.attn_norm = (
            mint.nn.LayerNorm(config.hidden_size, eps=config.norm_eps, bias=config.norm_bias)
            if layer_idx != 0
...
class ModernBertDecoderForCausalLM(ModernBertDecoderPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> from mindone.transformers import ModernBertDecoderForCausalLM
...
class ModernBertDecoderForSequenceClassification(ModernBertDecoderPreTrainedModel):
    def __init__(self, config: ModernBertDecoderConfig):
        ...
        self.classifier = mint.nn.Linear(config.hidden_size, config.num_labels, bias=config.classifier_bias)
