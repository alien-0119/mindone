...
from transformers import GraniteSpeechConfig, GraniteSpeechEncoderConfig
...
### Projector
class GraniteSpeechEncoderProjector(nn.Cell):
    def __init__(self, config: GraniteSpeechConfig):
        ...
        self.query = Parameter(mint.zeros((1, self.num_queries, config.projector_config.hidden_size)))
        self.query.data.normal_(mean=0.0, std=1.0)

        # By default, this will be a blip_2_qformer config
        self.qformer = AutoModel.from_config(config.projector_config)
        self.linear = mint.nn.Linear(config.projector_config.hidden_size, config.text_config.hidden_size)

    def construct(self, hidden_states: Tensor) -> Tensor:
        batch_size, seq_len, dim = hidden_states.shape
        nblocks = math.ceil(seq_len / self.window_size)
        pad = nblocks * self.window_size - seq_len
        hidden_states = mint.nn.functional.pad(hidden_states, (0, 0, 0, pad), "constant", 0)
...
class GraniteSpeechConformerFeedForward(nn.Cell):
    """Feedforward module for conformer encoder blocks."""

    def __init__(self, config: GraniteSpeechEncoderConfig):
        super().__init__()
        self.pre_norm = mint.nn.LayerNorm(config.hidden_dim)
        self.up_proj = mint.nn.Linear(config.hidden_dim, config.hidden_dim * config.feedforward_mult)
        self.silu = mint.nn.SiLU()
        self.dropout = mint.nn.Dropout(config.dropout)
        self.down_proj = mint.nn.Linear(config.hidden_dim * config.feedforward_mult, config.hidden_dim)
...
class GraniteSpeechConformerAttention(nn.Cell):
    ...
    def __init__(self, config: GraniteSpeechEncoderConfig):
        ...
        self.pre_norm = mint.nn.LayerNorm(config.hidden_dim)
        self.to_q = mint.nn.Linear(config.hidden_dim, inner_dim, bias=False)
        self.to_kv = mint.nn.Linear(config.hidden_dim, inner_dim * 2, bias=False)
        self.to_out = mint.nn.Linear(inner_dim, config.hidden_dim)
        self.rel_pos_emb = mint.nn.Embedding(2 * self.max_pos_emb + 1, self.dim_head)
        self.dropout = mint.nn.Dropout(config.dropout)
    ...
    def construct(self, hidden_states: Tensor, attention_dists: Tensor) -> Tensor:
        hidden_states = self.pre_norm(hidden_states)
        ...
        if remainder > 0:
            ...
            hidden_states = mint.nn.functional.pad(hidden_states, (0, 0, 0, self.context_size - remainder))
        ...
        # pos_attn = mint.sum(query_states.unsqueeze(-2) * rel_pos_emb_expanded, dim=-1) * self.scale
        # einsum implementation of pos_attn - gives x30 speedup over the alternative
        # TODO (@avihu111) find a fast alternative to einsum
        pos_attn = mint.einsum("b m h c d, c r d -> b m h c r", query_states, rel_pos_emb) * self.scale

        if remainder > 0:
            # masked attention in the extended block
            mask = mint.ones((self.context_size, self.context_size), dtype=ms.bool_)
            mask[:remainder, :remainder] = 0
            mask_value = -dtype_to_max(pos_attn.dtype)
...
class GraniteSpeechConformerDepthWiseConv1d(nn.Cell):
    ...
    def __init__(self, chan_in: int, chan_out: int, kernel_size: int):
        ...
        self.conv = mint.nn.Conv1d(chan_in, chan_out, kernel_size, groups=chan_in, bias=False)
...
class GraniteSpeechConformerConvModule(nn.Cell):
    ...
    def __init__(self, config: GraniteSpeechEncoderConfig):
        ...
        self.norm = mint.nn.LayerNorm(config.hidden_dim)
        self.up_conv = mint.nn.Conv1d(config.hidden_dim, inner_dim * 2, 1)
        ...
        self.silu = mint.nn.SiLU()
        self.batch_norm = mint.nn.BatchNorm1d(inner_dim)
        self.down_conv = mint.nn.Conv1d(inner_dim, config.hidden_dim, 1)
        self.dropout = mint.nn.Dropout(config.dropout)
...
class GraniteSpeechConformerBlock(nn.Cell):
    ...
    def __init__(self, config: GraniteSpeechEncoderConfig):
        ...
        self.post_norm = mint.nn.LayerNorm(config.hidden_dim)
...
class GraniteSpeechCTCEncoder(nn.Cell):
    def __init__(self, config: GraniteSpeechEncoderConfig):
        super().__init__()
        self.config = config

        # Precompute clamped relative positional encoding distances
        seq = mint.arange(config.context_size)
        relpos_dist = seq.view(-1, 1) - seq.view(1, -1)
        attention_dists = mint.clamp(relpos_dist, -config.context_size, config.context_size) + config.max_pos_emb
        self.register_buffer("attention_dists", attention_dists, persistent=False)
        self.input_linear = mint.nn.Linear(config.input_dim, config.hidden_dim, bias=True)
        self.layers = nn.CellList([GraniteSpeechConformerBlock(config) for _ in range(config.num_layers)])

        self.out = mint.nn.Linear(config.hidden_dim, config.output_dim, bias=True)
        self.out_mid = mint.nn.Linear(config.output_dim, config.hidden_dim, bias=True)
    ...
    def construct(self, hidden_states: Tensor):
        ...
        for idx, layer in enumerate(self.layers, start=1):
            ...
            if idx == self.num_layers // 2:
                ...
                hidden_states += self.out_mid(mint.nn.Softmax(dim=-1)(hidden_states_mid))
        return hidden_states
...
class GraniteSpeechPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: nn.Cell):
        ...
        elif isinstance(module, (mint.nn.LayerNorm, mint.nn.BatchNorm1d)):
...
class GraniteSpeechForConditionalGeneration(GraniteSpeechPreTrainedModel, GenerationMixin):
    ...
    def get_audio_features(self, input_features: Tensor) -> Tensor:
    ...
    def construct(
        ...
        input_features_mask: Optional[Tensor] = None,
        ...
        **lm_kwargs,
    ) -> Union[tuple[Tensor], GraniteSpeechCausalLMOutputWithPast]:
        r"""
        input_features_mask (`ms.Tensor`, *optional*):
        ...
        >>> from mindone.transformers import GraniteSpeechForConditionalGeneration
        ...
        >>> model_id = "ibm_granite/granite-speech-3.3-2b"
        ...
        >>> inputs["labels"] = processor(text=dataset[0]["text"], return_tensors="np").input_ids
        >>> inputs["labels"] = ms.tensor(inputs["labels"])
    ...
    def get_merged_audio_embeddings(
        self, input_ids: Tensor, audio_features: Tensor, input_features_mask: Optional[Tensor] = None
    ) -> Tensor:
        """
        ...
        if input_features_mask is not None:
            if mint.all(is_audio_index.int().sum(dim=1) != input_features_mask.int().sum(dim=1)).item():
