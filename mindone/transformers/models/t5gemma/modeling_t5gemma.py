...
from ...utils import TransformersKwargs, can_return_tuple, check_model_inputs, logging
from ...utils.generic import OutputRecorder

logger = logging.get_logger(__name__)


class T5GemmaRMSNorm(nn.Cell):
...
class T5GemmaRotaryEmbedding(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class T5GemmaSelfAttention(nn.Cell):
...
class T5GemmaCrossAttention(nn.Cell):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class T5GemmaClassificationHead(nn.Cell):
    """Head for sentence-level classification tasks."""

    def __init__(self, hidden_size: int, num_labels: int, classifier_dropout_rate: float = 0.0):
        super().__init__()
        self.dropout = mint.nn.Dropout(p=classifier_dropout_rate)
        self.out_proj = mint.nn.Linear(hidden_size, num_labels)
...
class T5GemmaAttention(nn.Cell):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
def bidirectional_mask_function(attention_mask: Optional[Tensor]) -> Callable:
    """
    ...
    def inner_mask(batch_idx: int, head_idx: int, q_idx: int, kv_idx: int) -> bool:
        if attention_mask is None:
            return mint.ones((), dtype=mindspore.bool)
        return attention_mask[batch_idx, kv_idx].to(mindspore.bool)
...
def make_default_2d_attention_mask(
    ...
    if token_ids is not None:
        ...
        attention_mask = (token_ids != pad_token_id).to(mindspore.long)
    else:
        attention_mask = mint.ones((hidden_states.shape[0], hidden_states.shape[1]), dtype=mindspore.long)
...
class T5GemmaEncoder(T5GemmaPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        cache_position = mint.arange(0, inputs_embeds.shape[1])
        ...
        normalizer = Tensor(self.config.hidden_size**0.5, dtype=hidden_states.dtype)
...
class T5GemmaModel(T5GemmaPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
        decoder_position_ids (`ms.Tensor` of shape `(batch_size, decoder_sequence_length)`, *optional*):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.config.pad_token_id is None:
            ...
            token_indices = mint.arange(input_ids.shape[-1], dtype=mindspore.int32)
            last_non_pad_token = (token_indices * non_pad_mask).argmax(-1)

            if self.config.is_encoder_decoder:
                last_non_pad_token += 1  # due to the right shift.
                last_non_pad_token = mint.clamp(last_non_pad_token, max=decoder_input_ids.shape[-1] - 1)
        else:
            last_non_pad_token = -1
            logger.warning_once(
