...
from ....models.utils import normal_
...
class ParakeetEncoderRelPositionalEncoding(nn.Cell):
    ...
    def __init__(self, config: ParakeetEncoderConfig):
        ...
        inv_freq = 1.0 / (
            base
            ** (
                mint.arange(0, config.hidden_size, 2, dtype=mindspore.int64).to(dtype=mindspore.float32)
                / config.hidden_size
            )
        )
    ...
    def construct(self, hidden_states: Tensor):
        ...
        position_ids = mint.arange(seq_length - 1, -seq_length, -1)
        inv_freq_expanded = self.inv_freq[None, :, None].float().expand((hidden_states.shape[0], -1, 1))
        ...
        pos_embed = mint.stack([sin, cos], dim=-1)
        pos_embed = pos_embed.reshape(*pos_embed.shape[:-2], -1)

        return pos_embed.to(dtype=hidden_states.dtype)


class ParakeetEncoderFeedForward(nn.Cell):
    def __init__(self, config: ParakeetEncoderConfig):
        super().__init__()
        self.linear1 = mint.nn.Linear(config.hidden_size, config.intermediate_size, bias=config.attention_bias)
        self.activation = ACT2FN[config.hidden_act]
        self.linear2 = mint.nn.Linear(config.intermediate_size, config.hidden_size, bias=config.attention_bias)
...
class ParakeetEncoderConvolutionModule(nn.Cell):
    def __init__(self, config: ParakeetEncoderConfig, module_config=None):
        ...
        self.pointwise_conv1 = mint.nn.Conv1d(channels, 2 * channels, kernel_size=1, stride=1, padding=0, bias=True)
        self.depthwise_conv = mint.nn.Conv1d(
            channels, channels, kernel_size, stride=1, padding=self.padding, groups=channels, bias=True
        )
        self.norm = mint.nn.BatchNorm1d(channels)
        self.pointwise_conv2 = mint.nn.Conv1d(channels, channels, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def construct(self, hidden_states, attention_mask=None):
        ...
        Args:
            hidden_states (`ms.Tensor` of shape `(batch, time, channels)`): Input tensor.
            attention_mask (`ms.Tensor` of shape `(batch, 1, time)`): Attention mask.

        Returns:
            `ms.Tensor`: Output tensor of shape `(batch, time, channels)`.
        ...
        if attention_mask is not None:
            all_masked_rows = mint.all(~attention_mask, dim=-1)
...
class ParakeetEncoderAttention(nn.Cell):
    ...
    def __init__(self, config: ParakeetEncoderConfig, layer_idx: int):
        ...
        # W_{k,R} projection
        self.relative_k_proj = mint.nn.Linear(
            config.hidden_size, config.num_attention_heads * self.head_dim, bias=False
        )
        # global content bias
        self.bias_u = Parameter(mint.zeros((config.num_attention_heads, self.head_dim)))
        # global positional bias
        self.bias_v = Parameter(mint.zeros((config.num_attention_heads, self.head_dim)))

    @deprecate_kwarg("past_key_value", new_name="past_key_values", version="4.58")
    def construct(
        self,
        hidden_states: Tensor,
        position_embeddings: Optional[Tensor],
    ...
    def _rel_shift(self, attention_scores):
        ...
        attention_scores = mint.nn.functional.pad(attention_scores, pad=(1, 0))
...
class ParakeetEncoderSubsamplingConv2D(nn.Cell):
    def __init__(self, config: ParakeetEncoderConfig):
        super().__init__()
        ...
        self.layers.append(
            mint.nn.Conv2d(1, self.channels, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding)
        )
        self.layers.append(mint.nn.ReLU())
        for i in range(self.num_layers - 1):
            ...
            # pointwise conv
            self.layers.append(mint.nn.Conv2d(self.channels, self.channels, kernel_size=1))
            # activation
            self.layers.append(mint.nn.ReLU())

        out_length = config.num_mel_bins // (self.stride**self.num_layers)
        self.linear = mint.nn.Linear(config.subsampling_conv_channels * out_length, config.hidden_size, bias=True)

    def _get_output_length(self, input_lengths: Tensor, conv_layer: mint.nn.Conv2d):
    ...
    def construct(self, input_features: Tensor, attention_mask: Tensor = None):
        ...
        for layer in self.layers:
            ...
            if isinstance(layer, mint.nn.Conv2d) and attention_mask is not None:
                current_lengths = self._get_output_length(current_lengths, layer)
                current_seq_length = hidden_states.shape[2]
                channel_mask = mint.arange(current_seq_length) < current_lengths[:, None]
...
class ParakeetEncoderBlock(GradientCheckpointingLayer):
    def __init__(self, config: ParakeetEncoderConfig, layer_idx: Optional[int] = None):
        ...
        self.norm_feed_forward1 = mint.nn.LayerNorm(config.hidden_size)
        self.norm_self_att = mint.nn.LayerNorm(config.hidden_size)
        self.norm_conv = mint.nn.LayerNorm(config.hidden_size)
        self.norm_feed_forward2 = mint.nn.LayerNorm(config.hidden_size)
        self.norm_out = mint.nn.LayerNorm(config.hidden_size)
...
class ParakeetPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, ParakeetEncoderAttention):
            ...
            normal_(module.bias_u, mean=0.0, std=std)
            normal_(module.bias_v, mean=0.0, std=std)

    def _get_subsampling_output_length(self, input_lengths: Tensor):
        ...
        for _ in range(num_layers):
            lengths = mint.div(lengths.to(dtype=mindspore.float32) + add_pad, stride) + 1.0
            lengths = mint.floor(lengths)

        return lengths.to(dtype=mindspore.int)

    def _get_output_attention_mask(self, attention_mask: Tensor, target_length: Optional[int] = None):
        """
        ...
        attention_mask = mint.arange(max_length) < output_lengths[:, None]
...
class ParakeetEncoder(ParakeetPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> from mindone.transformers import ParakeetEncoder
        ...
        position_embeddings = mint.nn.functional.dropout(
...
class ParakeetForCTC(ParakeetPreTrainedModel):
    ...
    def __init__(self, config: ParakeetCTCConfig):
        ...
        self.ctc_head = mint.nn.Conv1d(config.encoder_config.hidden_size, config.vocab_size, kernel_size=1)
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoProcessor
        >>> from mindone.transformers import ParakeetForCTC
        >>> from datasets import load_dataset, Audio
        >>> import mindspore as ms
        ...
        >>> inputs = processor(ds[0]["audio"]["array"], text=ds[0]["text"], return_tensors="np")
        >>> inputs = {k: ms.tensor(v) for k, v in inputs.items()}
        ...
        if labels is not None:
            ...
            attention_mask = (
                attention_mask if attention_mask is not None else mint.ones_like(input_features, dtype=mindspore.long)
            )
    ...
    def generate(
        ...
        **kwargs: Unpack[TransformersKwargs],
    ) -> Union[ParakeetGenerateOutput, Tensor]:
        r"""
        Example:
        ...
        >>> inputs = processor(ds[0]["audio"]["array"], text=ds[0]["text"], return_tensor="np")
        >>> inputs = {k: ms.tensor(v) for k, v in inputs.items()}
        ...
        outputs: CausalLMOutput = self.construct(
            input_features=input_features,
            attention_mask=attention_mask,
            **kwargs,
        )
