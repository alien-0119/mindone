...
class SEWNoLayerNormConvLayer(nn.Cell):
...
class SEWLayerNormConvLayer(nn.Cell):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id - 1] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class SEWGroupNormConvLayer(nn.Cell):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id - 1] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class SEWFeatureEncoder(nn.Cell):
...
class SEWFlashAttention2(SEWAttention):
    ...
    def _reshape(self, tensor: Tensor, seq_len: int, bsz: int):
...
class SEWSdpaAttention(SEWAttention):
    ...
    def construct(
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "SEWModel is using SEWSdpaAttention, but `scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
...
class SEWEncoder(nn.Cell):
    def __init__(self, config):
        ...
        self.layer_norm = mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = mint.nn.Dropout(config.hidden_dropout)
        self.layers = nn.CellList([SEWEncoderLayer(config) for _ in range(config.num_hidden_layers)])
    ...
    def construct(
        ...
        if attention_mask is not None:
            ...
            if self._use_flash_attention_2:
                ...
                attention_ids = (
                    mint.arange(0, max_encoder_length)
                    .view(1, -1)
