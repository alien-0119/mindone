...
class MimiOutput(ModelOutput):
    ...
    Args:
        ...
        encoder_past_key_values (`Cache`, *optional*):
            ...
            that can be used to speed up sequential decoding of the encoder transformer.
            This typically consists in the `past_key_values` returned by the model at a previous stage of decoding,
        ...
        decoder_past_key_values (`Cache`, *optional*):
            Pre-computed hidden-states (key and values in the self-attention blocks)
            that can be used to speed up sequential decoding of the decoder transformer.
            This typically consists in the `past_key_values` returned by the model at a previous stage of decoding,
            when `use_cache=True` or `config.use_cache=True`.
...
class MimiConv1d(nn.Cell):
    ...
    def __init__(
        ...
        stride = Tensor(self.conv.stride[0], dtype=mindspore.int64)
        dilation = self.conv.dilation[0]

        # Effective kernel size with dilations.
        kernel_size = Tensor((kernel_size - 1) * dilation + 1, dtype=mindspore.int64)
    ...
    def _pad1d(hidden_states: Tensor, paddings: Tuple[int, int], mode: str = "zero", value: float = 0.0):
        ...
        if not mode == "reflect":
            return mint.nn.functional.pad(hidden_states, paddings, mode, value)

        max_pad = max(padding_left, padding_right)
        extra_pad = 0
        if length <= max_pad:
            extra_pad = max_pad - length + 1
            hidden_states = mint.nn.functional.pad(hidden_states, (0, extra_pad))
        padded = mint.nn.functional.pad(hidden_states, paddings, mode, value)
...
class MimiMLP(nn.Cell):
    def __init__(self, config):
        ...
        self.fc1 = mint.nn.Linear(config.hidden_size, config.intermediate_size, bias=False)
        self.fc2 = mint.nn.Linear(config.intermediate_size, config.hidden_size, bias=False)
...
class MimiAttention(nn.Cell):
    ...
    def construct(
        ...
        attn_weights = mint.matmul(query_states, key_states.transpose(2, 3)) * self.scaling
...
class MimiSdpaAttention(MimiAttention):
    """
    Mimi attention module using scaled_dot_product_attention. This module inherits from
...
class MimiTransformerLayer(nn.Cell):
    def __init__(self, config: MimiConfig, layer_idx: int):
        ...
        self.input_layernorm = mint.nn.LayerNorm(config.hidden_size, eps=config.norm_eps)
        self.post_attention_layernorm = mint.nn.LayerNorm(config.hidden_size, eps=config.norm_eps)
    ...
    def construct(
        ...
        if cache_position is None:
            ...
            if isinstance(past_seen_tokens, Tensor):
                past_seen_tokens = past_seen_tokens.tolist()
...
class MimiEuclideanCodebook(nn.Cell):
    ...
    def decode(self, embed_ind):
        quantize = mint.nn.functional.embedding(embed_ind, self.embed)
        return quantize
...
class MimiResidualVectorQuantizer(nn.Cell):
    ...
    def encode(self, embeddings: Tensor, num_quantizers: Optional[int] = None) -> Tensor:
        ...
        out_indices = mint.stack(all_indices)
        return out_indices
...
class MimiSplitResidualVectorQuantizer(nn.Cell):
    ...
    def encode(self, embeddings: Tensor, num_quantizers: Optional[float] = None) -> Tensor:
        """
        Encode a given input tensor with the specified frame rate at the given number of quantizers / codebooks. The RVQ encode method sets
        the appropriate number of quantizers to use and returns indices for each quantizer.
        """
        ...
        if num_quantizers > self.max_num_quantizers:
            raise ValueError(
                f"The number of quantizers (i.e codebooks) asked should be lower than the total number of quantizers {self.max_num_quantizers}, "
                f"but is currently {num_quantizers}."
            )

        if num_quantizers < self.num_semantic_quantizers:
            raise ValueError(
                f"The number of quantizers (i.e codebooks) asked should be higher than the number of semantic quantizers {self.num_semantic_quantizers}, "
                f"but is currently {num_quantizers}."
            )
        ...
        if num_quantizers > self.num_semantic_quantizers:
            ...
            codes = mint.cat([codes, acoustic_codes], dim=0)
...
class MimiModel(MimiPreTrainedModel):
    ...
    def encode(
        ...
        if num_quantizers > self.config.num_quantizers:
            raise ValueError(
                f"The number of quantizers (i.e codebooks) asked should be lower than the total number of quantizers {self.config.num_quantizers}, "
                f"but is currently {num_quantizers}."
            )
    ...
    def construct(
        ...
        >>> from mindone.transformers.models.mimi import MimiModel
        ...
        >>> inputs = feature_extractor(raw_audio=audio_sample, return_tensors="np")
