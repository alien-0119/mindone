        ...
        **kwargs: Unpack[AllKwargsForChatTemplate],
        ...
        if kwargs.get("continue_final_message", False):
            if kwargs.get("add_generation_prompt", False):
                raise ValueError(
                    "continue_final_message and add_generation_prompt are not compatible. Use continue_final_message \
                        when you want the model to continue the final message, and add_generation_prompt when\
                              you want to add a header that will prompt it to start a new assistant message instead."
                )
